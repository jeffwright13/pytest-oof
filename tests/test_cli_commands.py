...
def test_export_results_sut_filters(runner, mock_db_client, temp_db):
    ...
    result = runner.invoke(cli, [
        "export", "results",
        "--db-path", str(temp_db),
        "--sut-id", SAMPLE_SUT_ID,
        "--sut-type", SAMPLE_SUT_TYPE,
        "--sut-version", SAMPLE_SUT_VERSION,
        "--sut-env", SAMPLE_SUT_ENV
    ])
    assert result.exit_code == 0
...
def test_delete_sessions_time_range(runner, mock_db_client, temp_db):
    ...
    result = runner.invoke(cli, [
        "analyze",
        "--db-path", str(temp_db),
        "--delete-after", after_time,
        "--delete-before", before_time
    ])
    assert result.exit_code == 0
