...
@pytest.fixture
def mock_results(mocker: MockerFixture, mock_test_result: TestResult, mock_session_metadata: SessionMetadata) -> Results:
...
def create_test_results():
    ...
    def _create_results(
        ...
        test_results.session_stats = TestSessionStats(
            num_tests=1,
            num_passes=1
        )
        ...
        return Results(
            ...
            session_stats=test_results.session_stats,
            report_stats=ReportBasedStats(),
            output_fields=OutputFields(),
            warnings=[],
            rerun_test_groups=[]
...
def test_get_test_status_changes(mocker: MockerFixture):
    ...
    mocker.patch(
        'pytest_oof.utils.datetime',
        **{'now.return_value': base_time}
    )
...
def test_find_test_changes(mocker: MockerFixture):
    ...
    sessions = []
    test_outcomes = [
        [("test_1", "PASSED"), ("test_2", "PASSED"), ("test_3", "PASSED")],  # Initial state
        [("test_1", "FAILED"), ("test_2", "PASSED"), ("test_3", "FAILED")],  # Some changes
        [("test_1", "PASSED"), ("test_2", "FAILED"), ("test_3", "FAILED")],  # More changes
    ]
    ...
    for i, outcomes in enumerate(test_outcomes):
        ...
        session_stats.num_passes = len([t for t in test_results if t.outcome == "PASSED"])
        session_stats.num_failures = len([t for t in test_results if t.outcome == "FAILED"])

        # Create report stats
        report_stats = ReportBasedStats()
        report_stats.num_tests = len(test_results)
        report_stats.num_passes = len([t for t in test_results if t.outcome == "PASSED"])
        report_stats.num_failures = len([t for t in test_results if t.outcome == "FAILED"])
...
def test_get_sut_runs(create_test_results):
    ...
    assert len(history.get_sut_runs(sut_id="app1", sut_type="web", sut_version="1.0.1")) == 1
...
def test_history_size_limit(create_test_results):
    ...
    assert [r.session_metadata.session_id for r in history.results] == ["session_2", "session_3", "session_4"]
    ...
    assert len(history.results) == 3
    assert [r.session_metadata.session_id for r in history.results] == ["session_4", "session_5", "session_6"]
