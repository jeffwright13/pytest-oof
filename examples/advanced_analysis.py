...
class AdvancedAnalyzer:
    ...
    def get_recently_failed_tests(self, hours: int = 24, min_failures: int = 1) -> List[Dict]:
    ...
    def get_recently_fixed_tests(self, hours: int = 24) -> List[Dict]:
        ...
        for result in results:
            ...
            if result["outcome"] == "failed":
                if (test_status[test_id]["last_failure"] is None or 
                    result["start_time"] > test_status[test_id]["last_failure"]):
                    test_status[test_id]["last_failure"] = result["start_time"]
            elif result["outcome"] == "passed":
                if (test_status[test_id]["last_pass"] is None or 
                    result["start_time"] > test_status[test_id]["last_pass"]):
        ...
        for test_id, data in test_status.items():
            if (data["last_failure"] and data["last_pass"] and 
                data["last_pass"] > data["last_failure"]):
                fixed_tests.append({
                    "test_id": test_id,
                    "last_failure": data["last_failure"],
            ...
            for sut_id in sut_ids:
                ...
                if stats:
                    ...
                    interval_stats["suts"][sut_id] = {
                        ...
                        "pass_rate": stat.total_passes / stat.total_tests if stat.total_tests > 0 else 0,
                        "failure_rate": stat.total_failures / stat.total_tests if stat.total_tests > 0 else 0,
                        "error_rate": stat.total_errors / stat.total_tests if stat.total_tests > 0 else 0,
                        "avg_reruns": stat.total_reruns / stat.total_tests if stat.total_tests > 0 else 0,
