...
@export.command()
@click.option('--output', type=click.Path(), help='Output file path (format determined by extension: .json or .jsonl)')
...
@click.option('--start-time', type=click.DateTime(), help='Start time for filtering (YYYY-MM-DD HH:MM:SS)')
@click.option('--end-time', type=click.DateTime(), help='End time for filtering (YYYY-MM-DD HH:MM:SS)')
@click.option('--outcome', type=click.Choice(['passed', 'failed', 'skipped', 'xfailed', 'xpassed']), help='Filter by test outcome')
...
def results(
    ...
    if output_path:
        if output_path.suffix == '.jsonl':
            ...
            console.print("[red]Error: Output file must have .json or .jsonl extension[/red]")
            raise click.Abort()
    ...
    try:
        ...
        if output:
            ...
            console.print("[yellow]No output file specified, results will be printed to stdout[/yellow]")
