...
JS_FILE = Path(__file__).parent.parent / "resources" / "scripts.js"
...
TABS_RESULTS_COLORS = {
    ...
    "Skips": ("hsl(172, 21%, 59%)", "rgba(128,128,0,0.5)"),
...
class TabContent:
    ...
    def fetch_raw_sections(self):
        ...
        self.add(
            "warnings_section", self.results.output_fields.warnings_summary.content
        )
        self.add("errors_section", self.results.output_fields.errors.content)
        self.add("passes_section", self.results.output_fields.passes.content)
        self.add("failures_section", self.results.output_fields.failures.content)
        self.add(
            "reruns_section", self.results.output_fields.rerun_test_summary.content
        )
...
class HtmlPage:
    ...
    def remove_tabs_without_content(self):
        ...
        if not self.results.test_results.all_tests():
            TABS_RESULTS.remove("All Tests")
        if not self.results.test_results.all_failures():
            TABS_RESULTS.remove("Failures")
        if not self.results.test_results.all_passes():
            TABS_RESULTS.remove("Passes")
        if not self.results.test_results.all_skips():
            TABS_RESULTS.remove("Skips")
        if not self.results.test_results.all_xpasses():
            TABS_RESULTS.remove("Xpasses")
        if not self.results.test_results.all_xfails():
            TABS_RESULTS.remove("Xfails")
        if not self.results.test_results.all_reruns():
    ...
    def create_testrun_results(self) -> str:
        return (
            ...
            + self.converter.convert(
                self.results.output_fields.lastline.content.replace("=", ""), full=False
            )
            + """</b></pre>"""
        )
    ...
    def create_final_test_summary(self) -> str:
        return (
            ...
            + self.converter.convert(
                self.results.output_fields.short_test_summary.content, full=False
            )
            + "\n"
            + self.converter.convert(
                self.results.output_fields.lastline.content, full=False
            )
            + """</pre></p></div>"""
        )
    ...
    def create_live_test_session_summary(self) -> str:
        search = re.search(
            ...
            self.results.output_fields.test_session_starts.content.encode(
                "unicode_escape"
            ).decode(),
        )
    ...
    def create_test_execution_info(self) -> str:
        ...
        d = {
            "Test run started": f"{self.results.session_start_time} UTC",
            "Test run completed": f"{self.results.session_stop_time} UTC",
            "Test run duration": f"{self.results.session_duration}",
            "This report generated": f"{now} UTC",
            "pytest-oof version": f"{__version__}",
        }
    ...
    def create_tabs(self) -> str:
        ...
        fold_visibility = "display: none;"

        if hasattr(self.results, "tui_regexfile") and self.results.tui_regexfile:
            tabs_links.extend(
                [
        ...
        # The "Full Output" tab (all output from the test run), as it occurred chronologically on the console
    ...
    def get_collapsible_results(self, outcome) -> str:
        ...
        if outcome == "reruns":
            results = []
            for group in self.results.rerun_test_groups:
                for result in group.full_test_list:
                    results.append(result)
        else:
            results = eval(f"self.results.test_results.all_{outcome}()")
        ...
        for result in results:
            ...
            content_html = (
                self.converter.convert(content_ansi, full=False)
                or "No output was produced for this test"
            )

            collapsible_results += f"""<button type="button" class="collapsible" style="border: none; outline: none;">{re.sub(r".[0-9]*$", "", str(result.start_time))} | {result.outcome} | {result.nodeid}</button> <div class="content"> <pre>{content_html}</pre> </div>"""
        return collapsible_results

    def get_metadata(self) -> str:
        lines = self.results.output_fields.test_session_starts.content.split("\n")
        try:
